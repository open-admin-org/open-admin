...
`OpenAdmin\Admin\Widgets\Box`用来生成box组件：
...
`OpenAdmin\Admin\Widgets\Collapse`类用来生成折叠插件：
```php
...
`OpenAdmin\Admin\Widgets\Form`类用来快速构建表单：
...
`OpenAdmin\Admin\Widgets\InfoBox`类用来生成信息展示块：
...
效果请参考后台首页的布局文件[HomeController.php](https://github.com/z-song/open-admin/blob/master/src/Console/stubs/HomeController.stub)的`index()`方法中，关于`InfoBox`的部分。

## Tab组件

`OpenAdmin\Admin\Widgets\Tab`类用来生成选项卡组件：
...
`OpenAdmin\Admin\Widgets\Table`类用来生成表格：
