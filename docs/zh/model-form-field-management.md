...
找到文件`app/Admin/bootstrap.php`,如果文件不存在，请更新`open-admin`，然后新建该文件
...
[wangEditor](http://www.wangeditor.com/)是一个优秀的国产的轻量级富文本编辑器，如果`open-admin`自带的基于`ckeditor`的编辑器组件使用上有问题，可以通过下面的步骤可以集成它，并覆盖掉`ckeditor`：
...
然后注册进`open-admin`,在`app/Admin/bootstrap.php`中添加以下代码：
...
>类中的静态资源也同样可以从外部引入，参考[Editor.php](https://github.com/z-song/open-admin/blob/1.3/src/Form/Field/Editor.php)
...
最后找到文件`app/Admin/bootstrap.php`,如果文件不存在，请更新`open-admin`，然后新建该文件,添加下面代码：
