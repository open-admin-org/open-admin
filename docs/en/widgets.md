...
`OpenAdmin\Admin\Widgets\InfoBox` class is used to generate the information presentation block:
...
Refer to the section on the `InfoBox` in the `index()` method of the home page layout file [HomeController.php](https://github.com/z-song/open-admin/blob/master/src/Console/stubs/HomeController.stub).

## Tab component

`OpenAdmin\Admin\Widgets\Tab` class is used to generate the tab components:
...
`OpenAdmin\Admin\Widgets\Table` class is used to generate tables:
