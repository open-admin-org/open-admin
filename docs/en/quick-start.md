...
php artisan admin:controller UserController App\\Models\\User

// options
php artisan admin:controller {model}
        {--title=}
...
> Where `uri` fills in the path part that does not contain the prefix of the route, such as the full path `http://localhost:8000/admin/demo/users`, just input `demo/users`, If you want to add an external link, just fill in the full url, such as `http://open-admin.org/`.
